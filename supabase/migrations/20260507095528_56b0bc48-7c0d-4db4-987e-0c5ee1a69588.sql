...
CREATE POLICY "Staff insert alerts" ON public.alerts FOR INSERT TO authenticated WITH CHECK (has_role(auth.uid(), 'doctor') OR has_role(auth.uid(), 'admin'));
...
CREATE INDEX idx_health_entries_patient_recorded ON public.health_entries(patient_id, recorded_date DESC);
